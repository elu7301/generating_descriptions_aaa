...
from .model import feature_extractor, tokenizer_gpt2, model, device
...
def generate_description_logic(input_text: str, image: PilImage) -> str:
    ...
    pix = feature_extractor(image, return_tensors="pt").to(device)
    text = tokenizer_gpt2(input_text, return_tensors="pt").to(device)
    ...
    description = description.split('Описание: ', maxsplit=1)[1].strip()
